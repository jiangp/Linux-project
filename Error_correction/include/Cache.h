...
public:
	...
	map<string, string >::iterator is_mapped(const string &word);
...
class Cache
{
	hash_map<string, string, compare()> m_cache;
	Mutex mutex;
public:
	Cache(const int &num = 100);
	hash_map<string, string >::iterator is_mapped(const string &word);
	void write_to_file(ostream &fout);
	void read_from_file(const string &file_name);
};
